...
NIMBLE=os.environ["NIMBLE_API_KEY"]; PROXY=os.environ["PROXY_API_BASE"].rstrip("/")
TOK=os.environ["KYLON_API_TOKEN"]; TODAY=datetime.date.today().isoformat()
...
def claude(p,mt=1500):
    r=http(f"{PROXY}/proxy/anthropic-vertex/v1/messages","POST",
        {"model":"claude-haiku-4-5","max_tokens":mt,"messages":[{"role":"user","content":p}]},
